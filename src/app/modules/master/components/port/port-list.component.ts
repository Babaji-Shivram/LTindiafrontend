  ...
  template: `
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between mb-4">
        <div>
          <h3 class="text-base font-medium text-gray-900">Port Management</h3>
          <p class="text-xs text-gray-600">Configure shipping ports and logistics hubs</p>
        </div>
        <button 
          [routerLink]="'/master/ports/new'"
      ...
      <div class="flex items-center justify-between space-x-4 mb-4">
        ...
        <select [(ngModel)]="typeFilter" class="px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-xs min-w-32">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr *ngFor="let port of getFilteredPorts()" class="hover:bg-gray-50">
                <td class="px-4 py-3 text-xs">
                  <span class="font-mono font-medium text-gray-900 bg-gray-100 px-2 py-0.5 rounded text-xs">{{ port.PortCode }}</span>
                </td>
                <td class="px-4 py-3">
                  <div class="text-xs font-medium text-gray-900">{{ port.PortName }}</div>
                </td>
                <td class="px-4 py-3">
                  <span [class]="getPortTypeBadge(port.PortType)">{{ port.PortType }}</span>
                </td>
                <td class="px-4 py-3 text-xs text-gray-900">{{ getCityName(port.CityId) }}, {{ getStateName(port.StateId) }}, {{ getCountryName(port.CountryId) }}</td>
                <td class="px-4 py-3">
                ...
                <td class="px-4 py-3 text-xs font-medium space-x-2">
                  <button 
                    [routerLink]="'/master/ports/' + port.lid + '/edit'"
                  ...
                  <button 
                    [routerLink]="'/master/ports/' + port.lid"
