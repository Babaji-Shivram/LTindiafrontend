  ...
  template: `
    <div class="bg-white rounded-lg border border-gray-200 p-6" *ngIf="port">
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          <h3 class="text-base font-medium text-gray-900">{{ port.PortName }}</h3>
          <p class="text-xs text-gray-600">Port Details</p>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-3">
            <div>
              <label class="block text-xs font-medium text-gray-500">Port Code</label>
              <p class="text-xs text-gray-900 font-mono bg-gray-100 px-2 py-1 rounded inline-block">{{ port.PortCode }}</p>
            </div>

            <div>
              <label class="block text-xs font-medium text-gray-500">Port Name</label>
              <p class="text-xs text-gray-900">{{ port.PortName }}</p>
            </div>

            <div>
              <label class="block text-xs font-medium text-gray-500">Port Type</label>
        ...
        <div class="space-y-4">
          <h4 class="text-xs font-medium text-gray-900 border-b border-gray-200 pb-2">Location Information</h4>
          
          <div class="space-y-3">
            <div>
              <label class="block text-xs font-medium text-gray-500">Country</label>
              <p class="text-xs text-gray-900">{{ getCountryName(port.CountryId) }}</p>
            </div>

            <div>
              <label class="block text-xs font-medium text-gray-500">State</label>
              <p class="text-xs text-gray-900">{{ getStateName(port.StateId) }}</p>
            </div>

            <div>
              <label class="block text-xs font-medium text-gray-500">City</label>
              <p class="text-xs text-gray-900">{{ getCityName(port.CityId) }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="mt-6 pt-4 border-t border-gray-200">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p class="text-xs text-gray-900">{{ port.CreatedDate | date:'medium' }}</p>
          </div>
          <div *ngIf="port.ModifiedDate">
            <label class="block text-xs font-medium text-gray-500">Last Updated</label>
            <p class="text-xs text-gray-900">{{ port.ModifiedDate | date:'medium' }}</p>
          </div>
        </div>
      </div>
    </div>
