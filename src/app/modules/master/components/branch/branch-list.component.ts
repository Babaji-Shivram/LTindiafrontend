  ...
  template: `
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between mb-4">
        <div>
          <h3 class="page-title text-gray-900">Branch Management</h3>
          <p class="secondary-text text-gray-600">Manage company branches and offices</p>
        </div>
        <button 
          routerLink="/masters/branches/new"
          ...
          <table class="w-full">
            <thead class="bg-gray-50">
              <tr>
                ...
                <th class="px-4 py-3 text-left table-header uppercase tracking-wider">Branch Name</th>
                <th class="px-4 py-3 text-left table-header uppercase tracking-wider">Location</th>
                <th class="px-4 py-3 text-left table-header uppercase tracking-wider">Manager</th>
                <th class="px-4 py-3 text-left table-header uppercase tracking-wider">Contact</th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              <tr *ngFor="let branch of getFilteredBranches()" class="hover:bg-gray-50">
                <td class="px-4 py-3 caption">
                  <span class="font-mono font-medium text-gray-900 bg-gray-100 px-2 py-0.5 rounded caption">{{ branch.BranchCode }}</span>
                </td>
                <td class="px-4 py-3">
                  <div>
                    <div class="caption font-medium text-gray-900">{{ branch.BranchName }}</div>
                    <div class="caption text-gray-500">{{ branch.BranchAddress }}</div>
                  </div>
                </td>
                <td class="px-4 py-3 caption text-gray-900">{{ branch.BranchState }}, {{ branch.BranchCountry }}</td>
                <td class="px-4 py-3 caption text-gray-900">{{ branch.BranchManager }}</td>
                <td class="px-4 py-3">
                  <div>
                    <div class="caption text-gray-900">{{ branch.BranchEmail }}</div>
                    <div class="caption text-gray-500">{{ branch.BranchPhone }}</div>
                  </div>
                </td>
                <td class="px-4 py-3">
