  ...
  template: `
    <div class="space-y-4">
      ...
      <div class="flex items-center justify-between mb-4">
        <div>
          <h3 class="page-title text-gray-900">State Management</h3>
          <p class="secondary-text text-gray-600">Manage states and provinces</p>
        </div>
      ...
      <div class="flex items-center justify-between space-x-4 mb-4">
        <div class="relative flex-1 max-w-md">
          <input type="text" 
                 ...
                 [(ngModel)]="searchTerm"
                 class="search-input w-full pl-10 pr-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
        ...
        <select [(ngModel)]="statusFilter" class="form-input px-3 py-1.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 min-w-32">
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            <thead class="bg-gray-50">
              <tr>
                <th class="table-header px-4 py-3 text-left uppercase tracking-wider">Code</th>
                <th class="table-header px-4 py-3 text-left uppercase tracking-wider">State Name</th>
                <th class="table-header px-4 py-3 text-left uppercase tracking-wider">Country</th>
                <th class="table-header px-4 py-3 text-left uppercase tracking-wider">Status</th>
                <th class="table-header px-4 py-3 text-left uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody class="bg-white divide-y divide-gray-200">
              <tr *ngFor="let state of getFilteredStates()" class="hover:bg-gray-50">
                <td class="table-cell px-4 py-3">
                  <span class="font-mono font-medium text-gray-900 bg-gray-100 px-2 py-0.5 rounded">{{ state.code }}</span>
                </td>
                <td class="px-4 py-3">
                  <div class="table-cell-important">{{ state.name }}</div>
                </td>
                <td class="table-cell px-4 py-3 text-gray-900">{{ state.countryName }}</td>
                <td class="px-4 py-3">
                  <span [class]="getStatusBadge(state.status)">{{ state.status }}</span>
                </td>
                <td class="table-cell px-4 py-3 space-x-2">
